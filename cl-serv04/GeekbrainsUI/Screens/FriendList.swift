...
class FriendsDatabase {
    static func getFriends() -> [Friend]{
        ...
        for a in 0...myVKUser.count-1 {
            myFriends.append(Friend(userName: myVKUser[a].firstName + " " + myVKUser[a].lastName, avatarPath: myVKUser[a].avatarPath, isOnline: myVKUser[a].isOnline, id: myVKUser[a].id))
        }
        return myFriends
    }
}
...
class FriendList: UITableViewController, UINavigationControllerDelegate {
    ...
    override func viewDidLoad() {
        ...
        vkAPI.getFriendList(token: Session.shared.token){           self.friends = FriendsDatabase.getFriends()
