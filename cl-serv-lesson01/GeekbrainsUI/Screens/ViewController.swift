...
class LoginViewController: UIViewController, UITextFieldDelegate //для фокусирования курсора на текстовое поле
{
 ...
 //       view.addGestureRecognizer(panGesture)
    ...
    @IBAction func RegPressed(_ sender: Any) {
        ...
        //Заглушечный код для ДЗ по синглтону
        guard let loginInput = Login2.text else {return}
        UserSession.defaultSession.id = 1
        UserSession.defaultSession.token = loginInput
        showLoginError(messagetext: "введенный логин = \(UserSession.defaultSession.token)")
        //конец заглушечного кода для ДЗ по синглтону
