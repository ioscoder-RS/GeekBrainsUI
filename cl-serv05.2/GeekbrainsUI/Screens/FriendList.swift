...
class FriendList: UITableViewController, UINavigationControllerDelegate {
    ...
    var presenter = FriendListPresenterImplementation(database: FriendCoreDataRepository(stack: CoreDataStack.shared), api: VKAPi())
    ...
    private func getFriendList(){
        ...
        vkAPI.getFriendList(token: Session.shared.token){  result in
            ...
            case .success(var webUsers): //массив VKUser из Web
                ...
                //в результате имеем массив пользователей webUsers, либо напрямую из Web, либо из БД (для databaseMode = true)
                
                //преобразовываем результат в формат структуры отображения = [Friend]
                self.friends = convertFriends(users: webUsers)
                //в closure перерисовываем tableView -  чтобы только по завершению вывести на экран
                self.initFriendSection()
                self.tableView.reloadData()
            case .failure(let error):
                print("we got error on vkAPI.getFriendList: \(error)")
            }//switch case
        }//completion
    }//func getGriendList
